...
def train():
    ...
    if data_args.dataset == "c4":
        ...
        dataloader, testloader = get_loaders(data_args.dataset,  model=model_args.model_name_or_path, seqlen=data_args.seqlen, seed=7)
...
#    model.seqlen = seqlen  #TODO
    ...
    for j, data in tqdm(enumerate(dataloader[:data_args.num_examples])):
        ...
        for i, layer in enumerate(_layers):
            ...
            k_proj, v_proj = get_modules_kv(layer)
            kgrad = (k_proj.act.grad).float().cpu()
            vgrad = (v_proj.act.grad).float().cpu()

            if f'k_proj{i}' not in grads_k:
                grads_k[f'k_proj{i}'] = kgrad
                acts_k[f'k_proj{i}'] = k_proj.act.data
            else:
                grads_k[f'k_proj{i}'] = torch.cat((grads_k[f'k_proj{i}'], kgrad), dim=1)
                acts_k[f'k_proj{i}'] = torch.cat((acts_k[f'k_proj{i}'], k_proj.act.data), dim=1)
            if f'v_proj{i}' not in grads_v:
                grads_v[f'v_proj{i}'] = vgrad
                acts_v[f'v_proj{i}'] = v_proj.act.data
            else:
                grads_v[f'v_proj{i}'] = torch.cat((grads_v[f'v_proj{i}'], vgrad), dim=1)
                acts_v[f'v_proj{i}'] = torch.cat((acts_v[f'v_proj{i}'], v_proj.act.data), dim=1)

    grads_k = torch.cat([v.to('cpu') for v in grads_k.values()], dim=0)
    grads_v = torch.cat([v.to('cpu') for v in grads_v.values()], dim=0)
    acts_k = torch.cat([v.to('cpu') for v in acts_k.values()], dim=0)
    acts_v = torch.cat([v.to('cpu') for v in acts_v.values()], dim=0)

    torch.save(acts_v, f"/data/zeyu/antkv/gradient/mistral/kv/cache_v.pth")
    torch.save(acts_k, f"/data/zeyu/antkv/gradient/mistral/kv/cache_k.pth")
    torch.save(grads_v, f"/data/zeyu/antkv/gradient/mistral/gradient/cache_v.pth")
    torch.save(grads_k, f"/data/zeyu/antkv/gradient/mistral/gradient/cache_k.pth")
